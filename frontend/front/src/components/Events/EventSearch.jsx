...
const EventSearch = ({ showAlert }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    // Filtrar parámetros que tienen valor
    const filteredParams = Object.fromEntries(
      Object.entries(localSearch).filter(([_, value]) => value !== '')
    )
  ...
  return (
    <div className="card p-6 mb-6">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div>
          <label htmlFor="startDate" className="block text-sm font-medium text-gray-700 mb-1">Fecha</label>
          <input
            type="date"
            id="startDate"
            name="startDate"
